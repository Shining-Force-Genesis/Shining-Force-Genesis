...
func play_turn():
	pself = Singleton_CommonVariables.battle__currently_active_actor.get_child(0)
	
	print("Singleton_CommonVariables.battle__turn_number ", Singleton_CommonVariables.battle__turn_number)
	
	if Singleton_CommonVariables.battle__turn_number == 1:
		for _i in range(5):
			pself.MoveInDirection("Up")
			await Signal(pself, "signal_move_complete")
...
#		Singleton_CommonVariables.target_selection_node.setup_use_range_and_target_range_selection_enemey_static()
#		Singleton_CommonVariables.target_selection_node.target_range.draw_cursor_at_position(res)
...
#	for character in Singleton_CommonVariables.character_nodes.get_children():
...
#			await Signal(self, "signal_complete_a_path_check")
#
...
#				#emit_signal("signal_logic_completed")
#				#pself.internal_call_complete()
#				await Signal(Singleton_CommonVariables.battle_scene_node, "signal_battle_complete_damage_step")
#
#
...
#
#
#	# await Signal(pself.get_tree().create_timer(0.3), "timeout")
#
#	emit_signal("signal_logic_completed")
#	pself.internal_call_complete()
#
...
#
#	var mpr = Singleton_CommonVariables.active_actor_move_point_representation
...
#	var sp = Singleton_CommonVariables.field_logic_node.tilemap.world_to_map(pself.position)
#	var ep = Singleton_CommonVariables.field_logic_node.tilemap.world_to_map(target_node_arg.position)
#
#	find_path(sp, ep)
#
...
#			await Signal(pself, "signal_move_direction_completed")
...
#	var tile_point
#	for enemey_actor in Singleton_CommonVariables.enemey_nodes.get_children():
#		tile_point = Singleton_CommonVariables.field_logic_node.tilemap.world_to_map(enemey_actor.position)
