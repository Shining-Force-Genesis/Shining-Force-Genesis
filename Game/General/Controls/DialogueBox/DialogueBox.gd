...
var dialogue_box_is_visible: bool = true
var wait_for_user_input_end: bool = false
...
func cust_show_dialogue() -> void:
	var start_position = Vector2(default_position.x, default_position.y + 120)
	position = start_position
	show()
	
	var ani_tween: Tween = get_tree().create_tween()
	ani_tween.tween_property(self, "position", default_position, 0.1)
...
func play_message(str_arg = "") -> void:
	...
	var parsed_data  = JSON.parse_string("{\"script\": [{\"Text\": \"" + str_arg + "\"}]}")
	dialogue_index = 0
	dialogue = parsed_data.script
...
func load_dialog():
	active = true
	# if active convo shouldnt be re-ran 
	# TODO: FIXME: POLISH later
	# cust_show_dialogue()
...
#				return
				...
				AudioManager.play_sfx(sound_to_play[1])
				...
				finished = true
				AudioManager.stop_dialogue_sfx()
				visible = false
				active = false
...
func ShowMenu(string_arg: String) -> void:
	if string_arg == "HQMenu":
		...
		print(Singleton_CommonVariables.interaction_node_reference.ITEM_LIST)
...
func GetTweenTimeForText(text_str: String) -> float:
	...
	# TODO: add in general settings different speeds - setting a min and max and possibliy a instant reveal
