...
func set_level(new_level: int) -> Array[String]:
	Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].level = new_level
	
	var stat_string_array: Array[String]= []
	
	print("Attack")
	var attack_increase = generate_stat_gain(
		new_level, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.attack, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.attack_target_unpromoted, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.attack_unpromoted_growth_curve
	)
	if attack_increase == 0:
		pass
	else:
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.attack += attack_increase
		stat_string_array.append("Attack increases by " + str(attack_increase) + ".")
	
	print("Defense")
	var defense_increase = generate_stat_gain(
		new_level, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.defense, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.defense_target_unpromoted, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.defense_unpromoted_growth_curve
	)
	if defense_increase == 0:
		pass
	else:
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.defense += defense_increase
		stat_string_array.append("Defense increases by " + str(defense_increase) + ".")
	
	print("Agility")
	var agility_increase = generate_stat_gain(
		new_level, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.agility, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.agility_target_unpromoted, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.agility_unpromoted_growth_curve
	)
	if agility_increase == 0:
		pass
	else:
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.agility += agility_increase
		stat_string_array.append("Agility increases by " + str(agility_increase) + ".")
	
	print("HP")
	var hp_increase = generate_stat_gain(
		new_level, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.hp, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.hp_target_unpromoted, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.hp_unpromoted_growth_curve
	)
	if hp_increase == 0:
		pass
	else:
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.hp += hp_increase
		stat_string_array.append("HP increases by " + str(hp_increase) + ".")
	
	print("MP")
	var mp_increase = generate_stat_gain(
		new_level, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.mp, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.mp_target_unpromoted, 
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.mp_unpromoted_growth_curve
	)
	if mp_increase == 0:
		pass
	else:
		Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.mp += mp_increase
		stat_string_array.append("MP increases by " + str(mp_increase) + ".")
	
	
	# TODO: need to check and do spell unlocks and spell level increase unlocks here
	
	print(stat_string_array)
	return stat_string_array


func generate_stat_gain(cur_lv: int, stat: int, stat_target: int, growth_curve):
	# print("Attack")
	# print(Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.attack)
	# print(Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.attack_target_unpromoted)
	# print(Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.attack_unpromoted_growth_curve)
	...
	var attack_curve = Singleton_CommonVariables.sf_game_data_node.ForceMembers[SF1_MEMBER_INDEX].stats.attack_unpromoted_growth_curve
	var percent_target_at_lvl = null
	
	for ap in attack_curve:
		if ap.x == cur_lv:
			percent_target_at_lvl = ap.y
			break
	...
	var stat_gain = 0
	if current_stat_percentage >= percent_target_at_lvl:
		# ahead of growth curve for stat don't increase this state right now
		# NOTE: this isn't entirely accurate to SF1 this is an ongoing best effort attempt
		pass
	else:
		# TODO: if severly behind allow for greater stat gains on level
